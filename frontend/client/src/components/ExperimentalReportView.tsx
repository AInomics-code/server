...
export default function ExperimentalReportView({ components, conversationHistory, messageIdx, healthScoresData, reportData, onQuestionClick, onQuestionSelect }: ExperimentalReportViewProps) {
  ...
  // Remove "New message" button aggressively
  useEffect(() => {
    const removeButton = () => {
      const buttons = document.querySelectorAll('button');
      buttons.forEach((btn) => {
        const text = btn.textContent?.trim() || '';
        if (text === 'New message' || text.includes('New message')) {
          const style = window.getComputedStyle(btn);
          if (style.position === 'fixed' || btn.getAttribute('style')?.includes('position: fixed')) {
            btn.remove();
          }
        }
      });
    ...
    removeButton();
    const interval = setInterval(removeButton, 50);
