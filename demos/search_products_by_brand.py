#!/usr/bin/env python3
"""
Search products by brand using vector similarity
Usage: python search_products_by_brand.py "lala"
"""
import sys
from utils import get_bedrock_client, get_embedding, search_vector_similarity


def search_products_by_brand(query_text, limit=1):
    """
    Search for products by brand using vector similarity
    
    Args:
        query_text (str): Search query
        limit (int): Number of results to return
    """
    print(f"\n🔍 Searching products by brand: '{query_text}'")
    print("=" * 70)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python search_products_by_brand.py 'search query'")
        print("\nExamples:")
        print("  python search_products_by_brand.py 'lala'")
        print("  python search_products_by_brand.py 'bimbo'")
        print("  python search_products_by_brand.py 'marcas de lácteos'")
        sys.exit(1)
    
    query = ' '.join(sys.argv[1:])
    search_products_by_brand(query)
